...
```bash
curl -X POST http://localhost:17789/api/tasks/{TASK_ID}/subagent \
...
```bash
curl -X POST http://localhost:17789/api/tasks/{TASK_ID}/activities \
  -H "Content-Type: application/json" \
  -d '{
...
curl -X POST http://localhost:17789/api/tasks/{TASK_ID}/deliverables \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X PATCH http://localhost:17789/api/tasks/{TASK_ID} \
  -H "Content-Type: application/json" \
  -d '{
    "status": "review"
  }'
...
```bash
curl -X PATCH http://localhost:17789/api/openclaw/sessions/{SESSION_ID} \
  -H "Content-Type: application/json" \
  -d '{
    "status": "completed",
...
```bash
curl -X DELETE http://localhost:17789/api/openclaw/sessions/{SESSION_ID}
```
