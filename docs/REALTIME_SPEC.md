...
const ws = new WebSocket('ws://localhost:17789/api/ws');
...
const task = await fetch('http://localhost:17789/api/tasks?status=inbox').then(r => r.json());

// 2. Log triage activity
await fetch(`http://localhost:17789/api/tasks/${task.id}/activities`, {
  method: 'POST',
  body: JSON.stringify({
...
// 3. Update status
await fetch(`http://localhost:17789/api/tasks/${task.id}`, {
  method: 'PATCH',
...
await fetch(`http://localhost:17789/api/tasks/${task.id}/subagent`, {
  method: 'POST',
  body: JSON.stringify({
...
await fetch(`http://localhost:17789/api/tasks/${task.id}/deliverables`, {
  method: 'POST',
  body: JSON.stringify({
